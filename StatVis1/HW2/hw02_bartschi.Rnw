...
data(olives)
@
It appears that there are \Sexpr{length(olives$Region)} observations in the data set, with \Sexpr{sum(olives$Region == "North")} observations in the North, \Sexpr{sum(olives$Region == "Sardinia")} observations in Sardinia, \Sexpr{sum(olives$Region == 'South')} observations in the South.
}
